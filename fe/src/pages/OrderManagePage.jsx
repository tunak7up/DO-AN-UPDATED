...
const getStatusBadgeClass = (status) => {
  switch (status) {
    case "Pending":
      return "status-badge pending"; // Vàng
    case "Processing":
      return "status-badge in-progress"; // Xanh dương
    case "Shipping":
      return "status-badge shipping"; // Tím/Cam
    case "Completed":
      return "status-badge completed"; // Xanh lá
    case "Cancelled":
      return "status-badge cancelled"; // Đỏ
    case "Not_Delivered":
      return "status-badge not-delivered";
    default:
      return "status-badge";
  }
};
...
function OrderManagePage() {
  ...
  const handleViewDetail = async (order) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/orders/${order.id}/history`,
        getHeaders(),
      );
      if (res.data.success) {
  ...
  const handleUpdateOrder = async () => {
    try {
      ...
      if (assigningShipper && assigningShipper !== selectedOrder.shipper_id) {
        await axios.put(
          `${API_URL}/orders/${selectedOrder.id}/assign`,
          {
      ...
      try {
        const res = await axios.get(
          `${API_URL}/orders/${selectedOrder.id}/history`,
          getHeaders(),
        );
        if (res.data.success) {
          setOrderHistories(res.data.data);
  ...
  const combinedHistories = [...orderHistories].sort(
          ...
          <div className="filter-dropdown">
            ...
            >
              ...
              <option value="Not_Delivered">
                Chưa gửi được hàng (Not Delivered)
              </option>
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <p>
                    <strong>Khu vực:</strong> {selectedOrder.shipping_district},{" "}
                ...
                >
                  <div className="form-group" style={{ marginBottom: "10px" }}>
                    ...
                    >
                      ...
                      <option value="Not_Delivered">
                        Not Delivered (Chưa gửi được hàng)
                      </option>
                    ...
                    >
                      ...
                      {shippers.map((shipper) => (
                        <option key={shipper.id} value={shipper.id}>
                          {shipper.name || shipper.fullname} ({shipper.phone})
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    >
                      ...
                      <>
                        <span style={{ color: "#0056b3" }}>
                          {" "}
                          {h.changer?.name || "Hệ thống"}
                        </span>{" "}
                        cập nhật: <br />
                        {h.old_status !== h.new_status &&
                          `Trạng thái: ${h.old_status || "Trống"} ➡️ ${h.new_status} `}
                        {h.old_payment_status !== h.new_payment_status &&
                          `| Thanh toán: ${h.old_payment_status || "Trống"} ➡️ ${h.new_payment_status} `}
                        {h.old_shipper_id !== h.new_shipper_id &&
                          `| Shipper: ${h.old_shipper?.name || "Chưa gán"} ➡️ ${h.new_shipper?.name || "Chưa gán"} `}
                        {h.note && (
                          <div>
