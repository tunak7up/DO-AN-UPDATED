...
exports.getMyAssignedOrders = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Lỗi lấy danh sách đơn giao hàng." });
  }
};
...
exports.updateShippingStatus = async (req, res) => {
  try {
    ...
    // Các action hợp lệ
    const validActions = ['assigned', 'picked_up', 'delivered', 'failed'];
    if (!validActions.includes(action)) {
      return res.status(400).json({ success: false, message: "Hành động không hợp lệ." });
    }

    const order = await Order.findOne({ where: { id: orderId, shipper_id: shipperId } });
    if (!order) {
    ...
    res.status(500).json({ success: false, message: "Lỗi hệ thống khi cập nhật giao hàng." });
  }
};
